============================
Installing the LOCKSS System
============================

.. note::

   This chapter describes how to install the LOCKSS |LATEST_MINOR| system from scratch. If you are upgrading an existing LOCKSS |PREVIOUS_MINOR| system to |LATEST_MINOR|, please see the upgrade instructions in the previous chapter:

   .. button-ref:: /upgrading/index
      :ref-type: doc
      :color: primary
      :outline:
      :align: center

.. tip::

   Before you begin installing the LOCKSS system, we strongly recommend you first bring your operating system up to date by applying security updates and upgrading installed packages. Ask your system administrator or see :doc:`/sysadmin/os-updates`.

**This chapter describes how to install the LOCKSS system from scratch.**

.. only:: html

   .. rubric:: Chapter Table of Contents

.. toctree::
   :maxdepth: 1

   user
   downloading
   running
